...
const parseGdelt = (events = []) =>
  (Array.isArray(events) ? events : [])
    ...
    .map((e) => ({
      ...
      source: "gdelt",
      severity:
        e.goldstein < -5 ? "critical" : e.goldstein < 0 ? "high" : "medium",
      color:
        e.goldstein < -5
          ? "rgba(239,68,68,0.9)"
          : e.goldstein < 0
            ...
            : "rgba(251,191,36,0.7)",
      ringColor:
        e.goldstein < -5 ? "rgba(239,68,68,0.6)" : "rgba(245,158,11,0.4)",
...
const parseFirms = (fires = []) =>
  (Array.isArray(fires) ? fires : [])
    .filter(
      (f) =>
        f.latitude && f.longitude && isInTacticalArea(f.latitude, f.longitude),
    )
    .slice(0, 80)
    .map((f) => ({
...
const parseFlights = (flights = []) => {
  const states = Array.isArray(flights?.states)
    ? flights.states
  ...
  return states
    ...
    .map((f) => {
      ...
      return {
        ...
        alt: parseFloat(
          isArr ? (f[7] || 0) / 1000000 : (f.altitude || 0) / 1000000,
        ),
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [threatsRes, flightsRes, driveRes] = await Promise.allSettled([
        ...
        fetch(`${API_BASE}/agente/drive/recent?max_results=30`, {
          headers,
          signal,
        }),
      ]);
