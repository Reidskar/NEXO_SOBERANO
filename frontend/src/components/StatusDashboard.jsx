...
import { RefreshCw, Activity, Shield, Cpu, Globe, Zap, Database, Radio, Server, Lock } from 'lucide-react';
...
function LiveTicker() {
  ...
  const items = [
    '→ Torre operativa · Backend FastAPI 8000 activo',
  ...
  return (
    <div style={{
      ...
      color: '#00e5ff', letterSpacing: '.08em',
      padding: '8px 20px', background: 'rgba(0,229,255,0.04)',
...
export default function StatusDashboard() {
  ...
  const fetchData = async () => {
    const [h, d] = await Promise.allSettled([
      fetch(`${API_BASE}/api/health`, { signal: AbortSignal.timeout(4000) }).then(r => r.json()),
      fetch(`${API_BASE}/api/tools/domain-scan`, { signal: AbortSignal.timeout(4000) }).then(r => r.json()),
    ]);
  ...
  return (
    <div ref={dashRef} style={{ maxWidth: 1100 }}>
      ...
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '28px 0 24px' }}>
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{
              ...
              background: backendOnline ? 'rgba(16,185,129,0.1)' : 'rgba(0,229,255,0.08)',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1, background: 'var(--border)', border: '1px solid var(--border)', marginBottom: 24 }}>
        ...
        <MetricCard icon={Lock} label="SSL" value={domain?.ssl_days_left ? `${domain.ssl_days_left}d` : 'TLS'} sub={domain?.ssl_valid ? 'Certificado válido' : 'Verificar certificado'} color="#00e5ff" />
        ...
        <MetricCard icon={Globe} label="API Status" value={backendOnline ? 'Online' : 'Local'} sub={health?.version || 'NEXO v3.0.0'} color={backendOnline ? '#10b981' : '#00e5ff'} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', padding: '24px 28px' }}>
          ...
          {!backendOnline && (
            <div style={{ marginTop: 14, padding: '8px 12px', background: 'rgba(0,229,255,0.04)', border: '1px solid rgba(0,229,255,0.1)', fontFamily: "'Space Mono', monospace", fontSize: 10, color: 'rgba(0,229,255,0.5)', letterSpacing: '.06em' }}>
        ...
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', padding: '24px 28px' }}>
          ...
          <ServiceRow name="Cloudflare Tunnel" status={backendOnline ? 'ok' : 'standby'} />
          {domain?.ips?.length > 0 && (
      ...
      {/* Sprint status */}
      <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', padding: '20px 28px' }}>
        <h2 style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, color: 'var(--cyan)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 16 }}>
          Sprint 1.3 · Módulos Activos
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          {[
            { name: 'OmniGlobe 3D', icon: Globe, status: 'ok', path: '/control/omniglobe' },
            { name: 'OSINT Engine', icon: Radio, status: 'ok', path: '/control/osint' },
            { name: 'Wireless Intel', icon: Zap, status: 'ok', path: '/control/wireless' },
            { name: 'Bóveda OSINT', icon: Database, status: 'ok', path: '/control/boveda' },
            { name: 'Mapa Global', icon: Globe, status: 'ok', path: '/control/mapa' },
            { name: 'Mercados', icon: Activity, status: 'ok', path: '/control/mercados' },
            { name: 'Escenarios IA', icon: Server, status: 'ok', path: '/control/escenarios' },
            { name: 'Video Estudio', icon: Cpu, status: 'ok', path: '/control/video-studio' },
            ...
            <a key={name} href={path} style={{ textDecoration: 'none' }}>
              <div style={{
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(0,229,255,0.25)'; e.currentTarget.style.background = 'rgba(0,229,255,0.05)'; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(0,229,255,0.08)'; e.currentTarget.style.background = 'rgba(0,229,255,0.02)'; }}
              >
                <Icon size={12} style={{ color: '#00e5ff', flexShrink: 0 }} />
                <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: 'var(--muted)', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{name}</span>
                <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#10b981', flexShrink: 0, boxShadow: '0 0 4px #10b981' }} />
              </div>
            </a>
          ))}
        </div>
